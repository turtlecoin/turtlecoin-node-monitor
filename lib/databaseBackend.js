...
DatabaseBackend.prototype.getNodeStats = function () {
  return new Promise((resolve, reject) => {
    ...
    query(this.db, 'SELECT * FROM `nodes` ORDER BY `name`', []).then((nodes) => {
      ...
      return query(this.db, 'SELECT `id`, ((SUM(`status`) / COUNT(*)) * 100) AS `availability` FROM `node_polling` GROUP BY `id`', [])
    }).then((rows) => {
      rows.forEach((row) => {
      ...
      return query(this.db, [
        'SELECT `id`, `status`, `timestamp` FROM `node_polling` ',
        'LEFT JOIN (SELECT `timestamp` AS `stamp` FROM `node_polling` GROUP BY `timestamp` ORDER BY `timestamp` DESC LIMIT 20) AS `last` ',
        '  ON `last`.`stamp` = `timestamp` ORDER BY `id` ASC, `timestamp` DESC'
      ].join(''), [])
    }).then((rows) => {
      const temp = {}
...
DatabaseBackend.prototype.saveNodesPolling = function (timestamp, nodeData) {
  ...
  nodeData.forEach((data) => {
    stmts.push({
      query: 'REPLACE INTO `node_polling` (`id`, `timestamp`, `status`, `feeAddress`, `feeAmount`, `height`, `version`, `connectionsIn`, `connectionsOut`) VALUES (?,?,?,?,?,?,?,?,?)',
      args: [
        data.id,
        timestamp,
        data.status,

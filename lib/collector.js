  ...
  constructor (opts) {
    ...
    this.pollingTimer.on('tick', () => {
      ...
      return Promise.all(promises)
        ...
        .then(() => this.emit('info', util.format('Saved polling event for %s nodes in the database', nodeListCache.length)))
        .catch(err => this.emit('error', util.format('Could not save polling event for %s nodes in the database: %s', nodeListCache.length, err.toString())))
    })

    this.updateTimer = new Metronome(this.updateInterval * 1000)
    this.updateTimer.pause = true
    this.updateTimer.on('tick', () => {
      return getList(this.nodeList)
        .then(list => { nodeListCache = list })
        .then(() => this.emit('update', nodeListCache))
        .catch(err => this.emit('error', util.format('Could not update the public node list: %s', err.toString())))
    })
    ...
    this.on('update', (nodes) => {
      return this.database.saveNodes(nodes)
        .then(() => this.emit('info', util.format('Saved %s nodes in the database', nodes.length)))
        .catch(err => this.emit('error', util.format('Could not save %s nodes in the database: %s', nodes.length, err.toString())))
    })
  }
  ...
  return request({
    ...
    .then(response => {
      if (response.nodes) {
        ...
        throw new Error('Node list not found')
      }
    })
}
